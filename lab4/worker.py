...
def execute(tasks: Queue, results: Queue):
    while True:
        if tasks.empty():
            break
        task = tasks.get()
        row, vec = task
        results.put(sum([row[i] * vec[i] for i in range(len(vec))]))
...
class MultiplicationWorker:
    def __init__(self, ip: str = "127.0.0.1", port: int = 2137):
        ...
        self.__tasks: Queue[tuple[list[float], list[float]]] = self.__manager.get_tasks()
        self.__results: Queue[float] = self.__manager.get_results()
